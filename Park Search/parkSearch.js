...
$(document).ready(function () {
  // Initialize the parkDropdown
  $("#parkDropdown2").select2({
    data: nationalParks.map((park) => ({
      id: park.LocationName,
      text: park.LocationName,
    })),
    // Other configurations for parkDropdown
  });

  // Initialize the thirdDropdown
  const parkTypes = [
    "Select One",
    "National Park",
    "National Monument",
  ...
  const parkTypeOptions = parkTypes.map((type) => ({
    id: type,
    text: type,
  }));

  // CSS and filter changes with 2????
  $("#thirdDropdown").select2({
    data: parkTypeOptions,
    // Other configurations for thirdDropdown
  });

  // Event listener for the thirdDropdown
  $("#thirdDropdown").on("change", function () {
    let selectedParkType = $(this).val();

    // Filter nationalParks based on the selected park type
    let filteredParks = [];
    if (selectedParkType !== "Select One") {
      filteredParks = nationalParks.filter(
        (park) => park.ParkType === selectedParkType
      );
    } else {
      filteredParks = nationalParks; // Show all parks if "Select One" is chosen
    }

    // Update parkDropdown options with filtered parks
    const parkDropdown = $("#parkDropdown");
    parkDropdown.empty(); // Clear existing options

    filteredParks.forEach((park) => {
      parkDropdown.append(
        $("<option></option>")
          .attr("value", park.LocationName)
          .text(park.LocationName)
      );
    });

    parkDropdown.trigger("change"); // Trigger change event after updating options
  });

  // Your event listeners and other code...
  ...
  stateDropdownEl.addEventListener("change", () => {
    ...
    populateParkDropdown(filterArray);

    // Append filtered parks to the page
  ...
  parkDropdownEl.addEventListener("change", () => {
    ...
    let filteredParks = nationalParks.filter(
      (park) => park.LocationName === parkValue
    );
    pTagEl.innerHTML = "";
    ...
    // Rest of your code to display filtered data...
    filteredParks.forEach((park) => {
      let parkDiv = document.createElement("div");
      ...
      let parkURL = document.createElement("p");
      let parkLocation = document.createElement("p");
      ...
      parkURL.appendChild(visitLink);
      parkDiv.appendChild(parkName);
      parkDiv.appendChild(parkLocation);
      parkDiv.appendChild(parkURL);
  ...
  // Function to populate park dropdown with options
  const populateParkDropdown = (array) => {
    parkDropdownEl.innerHTML = "";
    array.forEach((park) => {
      const parkOption = document.createElement("option");
      parkOption.value = park.LocationName; // Assuming LocationName is the value to display
  ...
  populateStateDropdown(nationalParks);
  populateParkDropdown(nationalParks);
});
